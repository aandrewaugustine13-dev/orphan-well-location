#!/usr/bin/env python3
"""Indiana: USGS NWIS fallback (maps.indiana.edu ArcGIS timed out)"""
...
IN_LAT_MIN, IN_LAT_MAX = 37.77, 41.77
IN_LON_MIN, IN_LON_MAX = -88.10, -84.78
NOW = datetime.utcnow()
...
def import_rows(rows, table, cc, nc):
    ...
    for s in range(0, total, BATCH_SIZE):
        ...
        else: err += len(batch); print(f"  [{end:>7}/{total}]  ERR {st}: {(e or '')[:80]}", file=sys.stderr)
        time.sleep(0.12)
    print(f"Done — {ins:,} sent, {err:,} errors.")
...
def fetch_usgs_gw(state_cd, state_name, lat_min, lat_max, lon_min, lon_max):
    ...
    for line in lines[hi+2:]:
        ...
        wells.append({"well_id":f"{state_cd.upper()}-USGS-{sn}","latitude":round(lat,6),"longitude":round(lon,6),"state":state_name,"county":gc(p,"county_cd"),"well_depth_ft":round(depth,1) if depth else None,"well_capacity_gpm":None,"water_use":gc(p,"water_use") or "Groundwater","status":"Active","year_constructed":None})
    print(f"  {state_name}: {len(wells)} GW sites.")
    return wells

if __name__ == "__main__":
    print("Indiana pipeline (USGS NWIS - maps.indiana.edu timed out)")
    print("no orphan well ArcGIS data found for Indiana")
    gw = fetch_usgs_gw("IN","Indiana",IN_LAT_MIN,IN_LAT_MAX,IN_LON_MIN,IN_LON_MAX)
    with open(os.path.join(SCRIPT_DIR,"in_groundwater_wells.csv"),"w",newline="",encoding="utf-8") as f:
    ...
    print("\n  UPDATE groundwater_wells SET geom = ST_SetSRID(ST_MakePoint(longitude, latitude), 4326) WHERE state = 'Indiana' AND geom IS NULL;")
    print(f"\nIN pipeline complete. Orphan: 0  GW: {len(gw):,}")
